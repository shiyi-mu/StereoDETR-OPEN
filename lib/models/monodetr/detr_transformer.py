...
class DETRTransformer(nn.Module):
    def __init__(
        ...
        self.decoder = Decoder(decoder_layer, num_decoder_layers, return_intermediate_dec, d_model, use_dab=use_dab, two_stage_dino=two_stage_dino)
...
class Decoder(nn.Module):
    ...
    def forward(self, tgt, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
                query_pos=None, src_padding_mask=None, mask_depth=None, bs=None, pos_embeds=None, attn_mask=None):
        output = tgt
        ...
        for lid, layer in enumerate(self.layers):
            ...
            output = layer(output,
                           ...
                           is_first=(lid == 0), pos_embeds=pos_embeds, self_attn_mask=attn_mask,query_pos_un=query_pos_un)

            # hack implementation for iterative bounding box refinement
            if self.bbox_embed is not None:
                tmp = self.bbox_embed[lid](output)
                if reference_points.shape[-1] == 6 or reference_points.shape[-1] == 8:
...
def build_detr_transformer_stereo(cfg):
