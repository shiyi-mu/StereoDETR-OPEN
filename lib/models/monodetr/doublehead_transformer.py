...
class DoubleHeadTransformer(nn.Module):
    def __init__(
            self,
            d_model=256,
            nhead=8,
        ...
        encoder_layer = VisualEncoderStereoLayer(
            d_model, dim_feedforward, dropout, activation, num_feature_levels, nhead, enc_n_points)
        self.encoder = VisualEncoderStereo(encoder_layer, num_encoder_layers)

        decoder_layer = DoubleHeadDecoderLayer(
            d_model, dim_feedforward, dropout, activation, num_feature_levels, nhead, dec_n_points, group_num=group_num)
        self.decoder = DoubleHeadDecoder(decoder_layer, num_decoder_layers, return_intermediate_dec, d_model, use_dab=use_dab, two_stage_dino=two_stage_dino)
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None, depth_pos_embed=None, depth_pos_embed_ip=None, attn_mask=None):
        ...
        for lvl, (src, mask, pos_embed) in enumerate(zip(srcs, masks, pos_embeds)):
            ...
            src_flatten_stereo.append(src)
            mask_flatten.append(mask)

        src_flatten_setreo = torch.cat(src_flatten_stereo, 1)
        ...
        # encoder
        memory_stereo = self.encoder(src_flatten_setreo, spatial_shapes, level_start_index, valid_ratios, lvl_pos_embed_flatten, mask_flatten)
        bs, _, c = memory_stereo.shape
        memory_left = memory_stereo[0:bs//2] 
        memory_right = memory_stereo[bs//2:] 
        memory = memory_left + memory_right
...
class VisualEncoderStereoLayer(nn.Module):
    def __init__(self,
    ...
    def forward(self, src_stereo, pos, reference_points, spatial_shapes, level_start_index, padding_mask=None):
        # self attention
        # src_stereo2 = self.self_attn(self.with_pos_embed(src_stereo, pos), reference_points, src_stereo, spatial_shapes, level_start_index, padding_mask)
        
        # cross attention
        batch_size_half = src_stereo.shape[0] // 2
        src_left = src_stereo[:batch_size_half]
        src_right = src_stereo[batch_size_half:]
        
        src_left2 = self.cross_attn(self.with_pos_embed(src_right, pos), reference_points, src_left, spatial_shapes, level_start_index, padding_mask)

        src_right2 = self.cross_attn(self.with_pos_embed(src_left, pos), reference_points, src_right, spatial_shapes, level_start_index, padding_mask)
        src_stereo2= torch.cat((src_left2, src_right2), dim=0)

        src_stereo = src_stereo + self.dropout1(src_stereo2)
        src_stereo = self.norm1(src_stereo)

        # ffn
        src_stereo = self.forward_ffn(src_stereo)

        return src_stereo


class VisualEncoderStereo(nn.Module):
...
class DoubleHeadDecoderLayer(nn.Module):
...
class DoubleHeadDecoder(nn.Module):
...
def build_double_head_transformer(cfg):
    return DoubleHeadTransformer(
